...
must_haves:
  truths:
    ...
    - "definition.pbism JSON contains model name, description, version, author (if available), and timestamps per user decision"
    - "Diagram layout visually separates fact tables from dimension tables with deterministic positioning"
...
- `generate_definition_pbism_json(model_name: str, description: str = "", author: str = "") -> str` - raises NotImplementedError. Per user decision, definition.pbism must include model name, description, version, author (if available), and timestamps.
...
- Contains `name` field matching model_name argument (per user decision: model name)
- Contains `description` field (per user decision: description)
- Contains `author` field when author argument is provided (per user decision: author if available)
- Contains `createdAt` timestamp field (per user decision: timestamps)
- Contains `modifiedAt` timestamp field (per user decision: timestamps)
- When author is empty string, author field is either omitted or empty (Claude's discretion)
- Output is deterministic: calling twice with same inputs and mocked time produces identical output
...
- For each relationship:
  ...
  - If `is_active is False`: `{indent_tmdl(1)}isActive: false` (omit for active, since true is default)
  - `{indent_tmdl(1)}fromColumn: {quote_tmdl_identifier(from_table_name)}.{from_column}` -- Note: in TMDL relationship syntax, table name portion is quoted with single quotes, column is not. Format: `'TableName'.ColumnName`. Extract table name from schema-qualified "schema.table" by taking just the table part for the TMDL reference.
  - `{indent_tmdl(1)}toColumn: {quote_tmdl_identifier(to_table_name)}.{to_column}`
...
**`generate_definition_pbism_json(model_name: str, description: str = "", author: str = "") -> str` in metadata.py:**
- Per user decision: "definition.pbism: Full metadata structure including model name, description, version, author (if available), timestamps"
...
```
- For timestamps: Accept an optional `timestamp: str | None = None` parameter. If None, use `datetime.now(timezone.utc).isoformat()`. If provided, use the given value. This ensures determinism in tests (pass a fixed timestamp) while providing real timestamps in production.
- If author is empty string, still include the key with empty value (keeps structure consistent and deterministic).
- IMPORTANT: This matches the user decision requiring model name, description, version, author, and timestamps. The previous minimal structure with only $schema/settings/version was insufficient.
...
- Build the output dict:
  ...
  - `"definition.pbism"` -> `generate_definition_pbism_json(model_name)` (description and author default to empty -- caller can provide them if available)
...
- definition.pbism JSON includes model name, description, version, author (if available), and timestamps per user decision
- diagramLayout.json positions tables with visual separation between facts and dimensions
