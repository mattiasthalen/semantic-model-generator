...
- `generate_partition_tmdl(table: TableMetadata, partition_name: str, catalog_name: str) -> str` - DirectLake partition with mode:directLake, entityName, schemaName, expressionSource referencing the catalog
...
<task type="auto">
  ...
  <action>
Implement all 6 functions in `src/semantic_model_generator/tmdl/generate.py`. Use Python f-strings only (no Jinja2) per user decision. Import and use Phase 2 utilities: `indent_tmdl` and `validate_tmdl_indentation` from `semantic_model_generator.utils.whitespace` (these already exist in that module), `quote_tmdl_identifier` from `semantic_model_generator.utils.identifiers`, `generate_deterministic_uuid` from `semantic_model_generator.utils.uuid_gen`, `map_sql_type_to_tmdl` from `semantic_model_generator.utils.type_mapping`.

NOTE: The `indent_tmdl(level: int) -> str` helper already exists in `src/semantic_model_generator/utils/whitespace.py` (created in Phase 2). It returns `"\t" * level`. Import it -- do NOT recreate it. Per user decision, this helper is used for clean indentation handling throughout generation.
...
- Column header: `{indent_tmdl(1)}column {quote_tmdl_identifier(column.name)}`
...
- Partition header: `{indent_tmdl(1)}partition {quote_tmdl_identifier(partition_name)} = entity`
